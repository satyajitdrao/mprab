function varargout = pso(fitnessfcn,nvars,Aineq,bineq,Aeq,beq,lb,ub,nonlcon,options)
%PSO    Constrained optimization using particle swarm optimization
%algorithm.
...
%
%   Copyright 2011- Institute for Dynamic Systems and Control, Department
%   of Mechanical and Process Engineering, ETH Zurich
%
%	The authors and copyright holders mentioned above cannot be held
%	responsible for any sort of loss or damange related to the use of this
%	software. Use at own risk.
...
if strcmpi(s.type,'anonymous') && strcmpi(options.Vectorized,'on')
	...
	'becuase fitness function is an anonymous function.'])
...
for k = options.InitialGeneration:options.Generations
	...
	[flag, g] =...
			psopenalize([state.pos; state.G],Aineq,bineq,Aeq,beq,lb,ub,nonlcon,options);
	
	% Always fall back on 'penalize' in case 'absorb' failes
...
for i = 1:length(copyfields)
	varargout{i} = psostruct.(copyfields{i});
end
...
for n = 1:size(pos,1)
    isempty(nonlcon)
	if isempty(nonlcon)
		[pos(n,:), cpl, exitflag(n)] =...
			linprog([],Aineq,bineq,Aeq,beq,lb,ub,pos(n,:),LocalOptions);
	else
		[pos(n,:), cpl, exitflag(n)] =...
	...
	% Update waitbar
% 	waitbar(n/(N/10),h,['Generating initial population ('...
...
function [state,exitflag] = psonearest(nvars,state,Aineq,bineq,Aeq,beq,lb,ub,nonlcon,options)
...
for i = ind'
	for j = 1:MaxIter
		...
		% Evaluate constraints
		[cpl, g] = psopenalize(mid,A,b,Aeq,beq,lb,ub,nonlcon,options);
